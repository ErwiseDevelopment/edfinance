...
<style>
    ...
    .amount-container { padding: 30px 0; text-align: center; position: relative; }
    #valor_display { 
        font-size: 3.5rem; font-weight: 800; border: none; background: transparent; 
        outline: none; width: 100%; text-align: center; color: var(--app-text-main); 
        transition: color 0.3s;
    }
    .amount-label { font-weight: 700; color: #94a3b8; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 1px; }

    /* Seletor Tipo (Receita/Despesa) */
    .type-picker { 
        background: #e2e8f0; padding: 5px; border-radius: 50px; 
        display: flex; gap: 0; margin-bottom: 30px; max-width: 350px; margin: 0 auto 30px auto; 
    }
    .type-picker .btn-check + .btn { 
        flex: 1; border: none; border-radius: 50px; padding: 10px; 
        font-weight: 700; color: #64748b; background: transparent; transition: 0.3s;
    }
    ...
    .input-app { 
        ...
        font-size: 1rem; font-weight: 600; color: #334155; height: 54px; transition: 0.3s;
    }
    .input-app:focus { 
        background: #fff !important; border-color: #4361ee !important; 
        box-shadow: 0 0 0 4px rgba(67,97,238,0.1); 
    }

    /* Tom Select Customizado */
    .ts-wrapper .ts-control { 
        background-color: #f8fafc !important; border: 1px solid #e2e8f0 !important; 
        border-radius: 16px !important; padding: 12px 16px !important; 
        font-size: 1rem !important; font-weight: 600 !important; min-height: 54px; 
        display: flex; align-items: center; 
    }
    .ts-dropdown { border-radius: 16px !important; border: none !important; box-shadow: 0 10px 30px rgba(0,0,0,0.1) !important; padding: 8px; z-index: 9999; }
    .ts-dropdown .option { padding: 10px 16px !important; border-radius: 10px; font-size: 0.95rem; margin-bottom: 2px; }
    .ts-dropdown .active { background-color: #f1f5f9 !important; color: #4361ee !important; font-weight: 700; }

    /* Switch Custom */
    .switch-container { 
        background: #f8fafc; border: 1px solid #e2e8f0; padding: 16px; border-radius: 20px; 
        display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; 
        cursor: pointer; transition: 0.2s; 
    }
    .switch-container:hover { border-color: #cbd5e1; }
    ...
    .btn-confirm { 
        ...
        font-weight: 700; border: none; flex: 2; transition: 0.2s; box-shadow: 0 10px 20px rgba(30, 41, 59, 0.2);
    }
    .btn-confirm:hover { background: #334155; transform: translateY(-2px); }
    
    .btn-add-more { 
        background: #fff; color: #1e293b; border: 1px solid #e2e8f0; border-radius: 16px; 
        padding: 16px; font-weight: 700; flex: 1; transition: 0.2s; 
    }
    .btn-add-more:hover { background: #f8fafc; border-color: #cbd5e1; }
    
    /* Feedback Visual */
    .badge-fatura { background-color: #dbeafe; color: #1e40af; padding: 8px 12px; border-radius: 10px; font-size: 0.85rem; font-weight: 700; margin-top: 10px; display: block; text-align: center; }

    #feedbackMeta { display: none; margin-top: 15px; background: #fff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 15px; }
    .meta-progress-bg { height: 8px; background: #f1f5f9; border-radius: 10px; overflow: hidden; margin: 8px 0; }
    .meta-progress-fill { height: 100%; transition: width 0.6s cubic-bezier(0.4, 0, 0.2, 1); border-radius: 10px; }
...
<div class="app-container">
    ...
    <div class="d-flex align-items-center mb-4">
        <a href="index.php?pg=cadastros" class="btn btn-light rounded-circle shadow-sm me-3 border" style="width: 45px; height: 45px; display: flex; align-items: center; justify-content: center;">
            <i class="bi bi-arrow-left"></i>
        </a>
        <div>
            <h4 class="fw-bold mb-0">Novo Lançamento</h4>
            <small class="text-muted">Preencha os dados abaixo</small>
        </div>
    </div>

    <form action="index.php?pg=salvar_conta_engine" method="POST" id="formLancamento">
        ...
        <div class="amount-container">
            ...
            <div class="d-flex justify-content-center align-items-baseline mt-2">
                ...
                <input type="text" id="valor_display" inputmode="decimal" placeholder="0,00" required autofocus autocomplete="off">
                <input type="hidden" name="contavalor" id="valor_real">
            </div>
        </div>

        <div class="type-picker shadow-sm">
            <input type="radio" class="btn-check" name="contatipo" id="entrada" value="Entrada" <?= $tipo_pre == 'Entrada' ? 'checked' : '' ?>>
            <label class="btn" for="entrada"><i class="bi bi-arrow-up-circle me-1"></i> Receita</label>
            
            <input type="radio" class="btn-check" name="contatipo" id="saida" value="Saída" <?= $tipo_pre == 'Saída' ? 'checked' : '' ?>>
            <label class="btn" for="saida"><i class="bi bi-arrow-down-circle me-1"></i> Despesa</label>
        </div>
            ...
            <div class="col-md-7">
                <div class="card-app">
                    <div class="mb-4">
                        ...
                        <input type="text" name="contadescricao" class="form-control input-app" placeholder="Ex: Mercado, Uber, Salário..." required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button type="button" class="btn input-app px-3 bg-white" data-bs-toggle="modal" data-bs-target="#modalRapidoCategoria" style="height: 54px; border: 1px solid #e2e8f0;">
                ...
                <div class="card-app">
                    <div class="switch-container" id="boxFixa" onclick="toggleFixa()">
                        <div>
                            ...
                            <small class="text-muted" style="font-size: 0.75rem;">Repete todo mês automaticamente</small>
                        </div>
                    ...
                    <div id="containerParcelas">
                        ...
                        <input type="number" name="contaparcela_total" id="inputParcelas" class="form-control input-app text-center fw-bold" value="1" min="1">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    formLancamento.addEventListener('submit', function(e) {
        ...
        if (!catVal) {
            ...
            if(tomCategoria) { tomCategoria.focus(); tomCategoria.open(); }
            return false;
        }
    });
    ...
    function verificarMeta() {
        ...
        fetch(AJAX_META_URL, { method: 'POST', body: formData })
    ...
    function salvarCategoriaRapida() {
        ...
        fetch(AJAX_CAT_URL, { method: 'POST', body: fd }).then(res => res.json()).then(data => {
