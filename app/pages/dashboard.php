...
// A conexão e o header já vêm carregados pelo "Porteiro" (public/index.php)
// Mas por garantia, usamos o APP_PATH se precisar chamar algo específico
...
// --- LÓGICA PHP ORIGINAL (Preservada) ---
...
// B. LISTAGEM DE ATRASADOS (Cartões)
$stmt_atrasos_cartao = $pdo->prepare("SELECT 
        car.cartonome, car.cartoid, c.competenciafatura,
...
<style>
    /* Variáveis já vêm do header, mas reforçamos caso precise */
    ...
    .month-pill { padding: 8px 16px; border-radius: 12px; background: white; border: 1px solid #e2e8f0; color: #64748b; text-decoration: none; font-weight: 600; font-size: 0.8rem; white-space: nowrap; transition: 0.2s; }
    .month-pill:hover { background: #f1f5f9; color: var(--primary); }
...
<div class="container py-4">
    ...
    <div class="mb-4">
        ...
        <p class="text-muted small">Acompanhe seu fluxo de caixa e projeções para este mês.</p>
    </div>
    ...
    <div class="d-flex overflow-x-auto gap-2 mb-4 pb-2" style="scrollbar-width: none;">
        ...
        ?>
            <a href="?pg=dashboard&mes=<?= $m ?>" class="month-pill <?= $mes_filtro == $m ? 'active' : '' ?>"><?= $label ?></a>
        <?php endfor; ?>
    </div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-12 col-md-5">
            <a href="?pg=faturas_geral" class="card-credit h-100 shadow-sm">
            ...
            ?>
                <div class="transaction-item atrasada-item shadow-sm">
                    ...
                    <div class="text-end">
                        ...
                        <a href="?pg=faturas&cartoid=<?= $fa['cartoid'] ?>&mes=<?= $fa['competenciafatura'] ?>" class="btn-action text-danger">Ver Fatura</a>
                    </div>
                </div>
            <?php endforeach; ?>
            ...
            <?php foreach($contas_atrasadas as $ca): ?>
                <div class="transaction-item atrasada-item shadow-sm">
                    ...
                    <div class="text-end">
                        ...
                        <a href="?pg=acoes_conta&acao=pagar&id=<?= $ca['contasid'] ?>" class="btn-action text-danger">Pagar Agora</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
        ...
        <?php foreach($contas_lista as $c): $pago = ($c['contasituacao'] == 'Pago'); ?>
            <div class="transaction-item shadow-sm">
                ...
                <div class="text-end">
                    ...
                    <a href="?pg=acoes_conta&acao=pagar&id=<?= $c['contasid'] ?>" class="btn-action" style="color: <?= $pago ? 'var(--success)' : 'var(--primary)' ?>;">
                        <?= $pago ? 'Pago' : 'Dar Baixa' ?>
                    </a>
                </div>
            </div>
            ...
            <?php foreach($cartoes_resumo as $cart): ?>
                <div class="transaction-item shadow-sm" style="border-left: 4px solid var(--warning);">
                    ...
                    <div class="text-end">
                        ...
                        <a href="?pg=faturas&cartoid=<?= $cart['cartoid'] ?>&mes=<?= $mes_filtro ?>" class="btn-action text-warning">Ver Detalhes</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
